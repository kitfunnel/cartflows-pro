...
class Cartflows_Pro_Gateway_Square {
		...
		if ( $this->key === $order->get_payment_method() ) {
			...
			if ( empty( $order->payment->token ) && $order->get_customer_id() < 1 ) {
				...
				try {
					...
					$response                  = $gateway->get_api()->create_customer( $order );
					$order->square_customer_id = $response->get_customer_id();
					$order->customer_id        = $response->get_customer_id();
					$response                  = $gateway->get_payment_tokens_handler()->create_token( $order );

				} catch ( Exception $e ) {
					wcf()->logger->log( '\n' . '==== Exception Occured ==== \n' . 'Can not create the token. Error Response: ' . print_r( $response, true ) ); //phpcs:ignore
				}
			}
		}
	...
	public function process_offer_payment( $order, $product ) {
		...
		$errors = $this->is_api_errors( $response );

		if ( $errors['is_error'] ) {
			wcf()->logger->log(
				'==== API Error: ====' . PHP_EOL . print_r( $errors, true ) . PHP_EOL . '==== Gateway Response start ====' . PHP_EOL . print_r( $response, true ) . PHP_EOL . '==== Error Gateway response end ====' ); //phpcs:ignore
		} else {
	...
	public function process_offer_refund( $order, $offer_data ) {
		...
		if ( ! is_null( $offer_data['refund_amount'] ) ) {
			...
			$errors = $this->is_api_errors( $refund_response );

			if ( $errors['is_error'] ) {

				// For support team, then can ask the user to set WP_DEBUG to true to log the error message.
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'CartFlows Square gateway refund failed: Response: ' . PHP_EOL . print_r( $errors, true ) ); //phpcs:ignore
				}
			} else {
	 ...
	 * @since x.x.x
	 */
	public function is_api_errors( $response ) {
