...
class Cartflows_Pro_Checkout_Meta_Data {
	...
	public function meta_fields_react( $settings, $step_id, $options ) {
		...
		$settings['settings']['multiple-order-bump-settings']['fields'] = array(
			...
			'wcf-ob-replace'           => array(
				...
				'desc'  => sprintf(
					...
					__( 'It will replace the first selected product (from checkout products) with the order bump product. %1$1sLearn More »%2$2s', 'cartflows-pro' ),
		...
		if ( class_exists( 'Cartflows_Helper' ) && Cartflows_Helper::get_global_setting( '_cartflows_store_checkout' ) === $flow_id ) {
			unset( $settings['settings']['multiple-order-bump-settings']['fields']['wcf-ob-replace-heading'] );
			unset( $settings['settings']['multiple-order-bump-settings']['fields']['wcf-ob-replace'] );
		}
